...
function prerenderTypes() {
  ...
  store.types.forEach((type, i) => {
    if (type.avatarType === AvatarType.CHAR) {
      let metrix = ctx.measureText(type.avatar);
      ...
      ctx.font = baseFontSize * charWidth / w + 'px Calibri';
      metrix = ctx.measureText(type.avatar);
      w = metrix.actualBoundingBoxRight - metrix.actualBoundingBoxLeft;
      h = metrix.actualBoundingBoxDescent + metrix.actualBoundingBoxAscent;

      const startX = i * charWidth;
      const startY = 0;

      ctx.beginPath();
      ctx.fillText(type.avatar, startX + (charWidth - w) / 2, startY + (charHeight - h) / 2 + metrix.actualBoundingBoxAscent);

      type.cacheImage = {x: i * charWidth, y: 0, w: charWidth, h: charHeight};
    }
  });
}
...
function draw() {
  ...
  for (let i = 0; i < characters.length; i++) {
    ...
    if (offscreenCtx && charType.cacheImage) {
      const x = char.position.x;
      const y = char.position.y;
      // const x = ~~(char.position.x + 0.5);
      // const y = ~~(char.position.y + 0.5);

      ctx.drawImage(offscreenCtx.canvas, charType.cacheImage.x, charType.cacheImage.y, charType.cacheImage.w, charType.cacheImage.h
